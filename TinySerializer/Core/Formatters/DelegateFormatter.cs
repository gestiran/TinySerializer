...
namespace TinySerializer.Core.Formatters {
    public class DelegateFormatter<T> : BaseFormatter<T> where T : class {
        ...
        protected override void DeserializeImplementation(ref T value, IDataReader reader) {
            ...
            if (invocationList != null) {
                ...
                try {
                    ...
                    reader.Context.Config.DebugContext.LogError("Recombining delegate invocation list upon deserialization failed with an exception of type "
                ...
                if (combinedDelegate != null) {
                    try {
                        ...
                        reader.Context.Config.DebugContext.LogWarning("Could not cast recombined delegate of type " + combinedDelegate.GetType().GetNiceFullName()
                                                                      + " to expected delegate type " + DelegateType.GetNiceFullName() + ".");
                    }
                }
            ...
            if (methodInfo == null) {
                if (useSignature) {
                    reader.Context.Config.DebugContext.LogWarning("Could not find method with signature " + name + "("
                                                                  + string.Join(", ", signature.Select(p => p.GetNiceFullName()).ToArray()) + ") on type '" + declaringType.FullName
                                                                  + (wasAmbiguous ? "; resolution was ambiguous between multiple methods" : string.Empty) + ".");
                } else {
                    reader.Context.Config.DebugContext.LogWarning("Could not find method with name " + name + " on type '" + declaringType.GetNiceFullName()
                                                                  + (wasAmbiguous ? "; resolution was ambiguous between multiple methods" : string.Empty) + ".");
                }
            ...
            if (methodInfo.IsGenericMethodDefinition) {
                ...
                try {
                    ...
                    reader.Context.Config.DebugContext.LogWarning("Method '" + declaringType.GetNiceFullName() + "." + methodInfo.GetNiceName()
                                                                  ...
                                                                  + string.Join(", ", genericArguments.Select(p => p.GetNiceFullName()).ToArray())
                                                                  + "'. Method creation failed with an exception of type " + ex.GetType().GetNiceFullName() + ", with the message: "
                                                                  + ex.Message);
            ...
            if (value == null) {
                reader.Context.Config.DebugContext.LogWarning("Failed to create delegate of type " + delegateType.GetNiceFullName() + " from method '"
                                                              + declaringType.GetNiceFullName() + "." + methodInfo.GetNiceName() + "'.");
        ...
        protected override void SerializeImplementation(ref T value, IDataWriter writer) {
            ...
            if (methodInfo.IsGenericMethodDefinition) {
                writer.Context.Config.DebugContext.LogError("Cannot serialize delegate made from the unresolved generic method definition " + methodInfo
